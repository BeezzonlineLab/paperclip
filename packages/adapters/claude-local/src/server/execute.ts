...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Claude requires UUID session IDs — reject non-UUID formats from adapter switches (e.g. OpenCode "ses_..." IDs)
  const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  const isValidClaudeSessionId = UUID_RE.test(runtimeSessionId);
